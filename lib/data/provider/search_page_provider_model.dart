...
class SearchPageProviderModel
  ...
  Future<Result<List<SearchPageListItemModel>>> _builder(
          Future<Result<List<Location>>> future) =>
      future.then((Result<List<Location>> value) {
        if (value.isValue) {
          return Result<List<SearchPageListItemModel>>.value(value.asValue.value
              .map((Location element) =>
                  SearchPageListItemModel(element.woeid, element.title))
              .toList());
        } else if (value.isError) {
          return Result<List<SearchPageListItemModel>>.error(
              value.asError.error);
        } else {
          return Result<List<SearchPageListItemModel>>.error(
              UnknownException());
